...
consumer_key = 'SRvTyFXLc60i0x9f8qMAT2MpU'
consumer_secret = 'R1gTfufwUWXIAVisryG5mYiXjoxne12x5lppDwgyajsZc0kfLu'
access_token = '385495280-gjzvbWZb0K5QYdjGB36DKNmrqV7omOcjMYd4YL16'
access_token_secret = 'LoHeYslsK28L6WiiHhzSuLvqCfvq1M1yOyqQ9MacBW1Jx'
...
def add_all_friends(twitter, users):
    ...
    for u in users:
       s = u['screen_name']
       friends = get_friends(twitter,s)
       u['friends'] = {r for r in friends} 
...
def print_num_friends(users):
    ...
    ###TODO
    for u in users:
        print (' %s - %d ' %(u['screen_name'],u['friends_count']))
...
def friend_overlap(users):
    ...
    for i in range(0,len(users)-1):
        for j in range(i+1,len(users)):
            co = set(users[i].get('friends')).intersection((users[j].get('friends')))
           # N = len(common)
            fo.append((users[i].get('screen_name'),users[j].get('screen_name'),len(co)))
            
    fs = sorted(sorted([d for d in fo if d[2]==d[2]],key=lambda x:x[0]),key=lambda x:x[2],reverse = True)
    return fs
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    for i in range(len(users)):
        if users[i].get('screen_name') == h:
            hc = get_friends(twitter,users[i]['screen_name'])
        elif users[i].get('screen_name')== d:
            dt = get_friends(twitter,users[i]['screen_name'])
    
    co = set(hc).intersection(dt)
    
    req = robust_request(twitter, 'users/show', {'user_id' : co})
    u = [r for r in req]
    
    un = list(map (lambda x: x['screen_name'],u))
    return un
    pass
...
def create_graph(users, friend_counts):
    ...
    user_name = []
    for i in range(0,len(users)):
        user_name = users[i]['screen_name']
        graph.add_node(user_name)
    
    [graph.add_node(i) for i in friend_counts if friend_counts[i] > 1]
      
            
    for i in range(0,len(users)):
        for j in graph.nodes():
            if j in users[i]['friends']:
                graph.add_edge(users[i]['screen_name'],j)
...
def draw_network(graph, users, filename):
    ...
    pos = {}
    for u in users:
        pos[u['screen_name']] = u['screen_name']
        
    fig = plt.figure(figsize=(20,20))
    nx.draw_networkx(graph,labels=pos,alpha=.7, width=.4,node_size=100)
    plt.show()
    plt.axis("off")
    fig.savefig('network_test.png')
